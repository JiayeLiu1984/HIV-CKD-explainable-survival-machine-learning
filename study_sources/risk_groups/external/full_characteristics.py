...
OUT=Path(__file__).resolve().parent;ROOT=Path('__CKD_WORKDIR__');S3=ROOT/'重庆外部验证_step5_GLU更新'
...
fg=json.loads((ROOT/'rolling_5y_step3_raw_features/feature_groups.json').read_text());cont=np.load(S3/'continuous_raw_0_60.npy',mmap_mode='r');binary=np.load(S3/'binary_raw_0_60.npy',mmap_mode='r');cat=np.load(S3/'categorical_raw_0_60.npy',allow_pickle=True);mask=np.load(S3/'sequence_row_mask.npy')
m=pd.read_csv(ROOT/'重庆外部验证_step6_LSTM_v2/super_landmark_external_metadata.csv');a=pd.read_csv(OUT.parent/'tables/patient_landmark_assignments.csv');a=a[a.model=='LSTM-v2'];m['landmark_year']=m.landmark_month//12
...
source=ROOT/'重庆_6month_panel_step4_GLU_updated_UTF8.csv'
raw=pd.read_csv(source,encoding='utf-8-sig',usecols=['ID','time_bin','SCr','Age','eGFR'],low_memory=False)
...
for lm in [0,5]:
    d=m[m.landmark_year==lm].copy();gid=d.local_patient_index.to_numpy(int);step=int(lm*2);ok=mask[gid,step].astype(bool);groups=d.group.to_numpy(int)
    ...
    source_agreement={v:int(np.isclose(x[v].astype(float),match[v].to_numpy(float),atol=1e-4,rtol=1e-5,equal_nan=False).sum()) for v in ['Age','eGFR']}
    # Supplementary SCr is independently linked, never back-calculated.
    # Require unique ID/time and matching age/eGFR; source labs may be missing
    # before imputation, so other source labs are not substituted for model inputs.
    for v in ['Age','eGFR']:compatible&=np.isclose(x[v].astype(float),match[v].to_numpy(float),atol=1e-4,rtol=1e-5,equal_nan=False)
    compatible&=np.isfinite(match.SCr.to_numpy(float))
...
(OUT/'data_audit.json').write_text(json.dumps({'reference_sha256':hashlib.sha256(REF.read_bytes()).hexdigest(),'source_SCr':str(source),'qa':qa,'grouping':'development landmark-specific tertile cutpoints transferred unchanged; frozen external predictions'},indent=2),encoding='utf8')
