...
FORMAL_STEP12A_PIPELINE_VERSION = "v4_formal_ig_explanation"
...
PIPELINE_VERSION = "v3_patient_6landmark_clinically_coherent_local_ig"
POSTPROCESS_REVISION = "v3_external_clinically_coherent_representatives_plus_clinical_trajectory_figure"
OUTPUT_DIR = (
    PROJECT_DIR
    / "rolling_5y_step12i_patient_dynamic_explanation_6landmark_local_ig_v3_clinically_coherent"
)
...
PATIENT_FIGURE_PNG = FIGURE_DIR / "Figure_Patient_Dynamic_Explanation_6Landmarks.png"
PATIENT_FIGURE_PDF = FIGURE_DIR / "Figure_Patient_Dynamic_Explanation_6Landmarks.pdf"
TRAJECTORY_OVERVIEW_PNG = FIGURE_DIR / "Figure_Patient_Trajectory_Overview_6Landmarks.png"
TRAJECTORY_OVERVIEW_PDF = FIGURE_DIR / "Figure_Patient_Trajectory_Overview_6Landmarks.pdf"


# =============================================================================
# Externally frozen, clinically coherent case selection from the dedicated
# preselection step.  This file is the ONLY source of the three patient IDs.
# =============================================================================
CLINICAL_CASE_DIR = (
    PROJECT_DIR
    / "rolling_5y_model_interpretation_final_6landmarks"
    / "FIGURE3_PATIENT_SELECTION_FINAL"
)

CLINICAL_CASE_SELECTION_FILE = (
    CLINICAL_CASE_DIR
    / "Figure3_selected_representative_cases_FINAL.csv"
)

CLINICAL_TRAJECTORY_FILE = (
    CLINICAL_CASE_DIR
    / "Figure3_selected_clinical_trajectory_long_FINAL.csv"
)

CLINICAL_FULL_CANDIDATE_AUDIT_FILE = (
    CLINICAL_CASE_DIR
    / "Figure3_full_candidate_clinical_audit_FINAL.csv"
)

FINAL_PATIENT_FIGURE_PNG = (
    FIGURE_DIR
    / "Figure3_Three_Representative_Patients_Dynamic_Explanation_FINAL.png"
)

FINAL_PATIENT_FIGURE_PDF = (
    FIGURE_DIR
    / "Figure3_Three_Representative_Patients_Dynamic_Explanation_FINAL.pdf"
)

FINAL_PATIENT_FIGURE_SOURCE = (
    OUTPUT_DIR
    / "Figure3_Three_Representative_Patients_Dynamic_Explanation_source.csv"
)

FINAL_PATIENT_TOP_IG_SOURCE = (
    OUTPUT_DIR
    / "Figure3_Three_Representative_Patients_TopLocalIG_source.csv"
)

TOP_LOCAL_HEATMAP_N = 6

CASE_DEFINITIONS = [
    ("Case A", "Stable low-risk with preserved renal function"),
    ("Case B", "Increasing risk with progressive renal deterioration"),
    ("Case C", "Persistently higher-risk, clinically stable profile"),
]
...
def build_or_load_sample_selection(
    ...
    """
    Load the three already-frozen clinically coherent representative cases and
    expand them to 3 patients × 6 formal landmarks = 18 OOF explanation origins.

    IMPORTANT
    ---------
    * This function DOES NOT reselect patients.
    * Patient IDs come only from Figure3_selected_representative_cases_FINAL.csv.
    * Clinical values were used upstream to define the illustrative phenotypes.
    * IG and CKD outcome were NOT used upstream for selection.
    * The function independently reconstructs the exact Step11 calibrated risk
      trajectory and verifies it against the selection file before any IG is run.
    """
    ...
    if len(selected_cases) != 3:
        raise ValueError(
            f"病例选择文件必须恰好3行，实际={len(selected_cases)}。"
        )
    ...
    if set(selected_cases["case_label"].astype(str)) != expected_case_labels:
        raise ValueError(
            "病例选择文件必须恰好包含Case A、Case B、Case C。"
        )

    if selected_cases["patient_local"].duplicated().any():
        raise ValueError("三名代表病例patient_local存在重复。")

    selected_cases = (
        selected_cases
        .sort_values("case_label")
    ...
    # -------------------------------------------------------------------------
    # Patient index / fold / landmark eligibility audit.
    # -------------------------------------------------------------------------
    for row in selected_cases.itertuples(index=False):
        patient_local = int(row.patient_local)

        if patient_local < 0 or patient_local >= len(common.development_idx):
            raise ValueError(
                f"{row.case_label}: patient_local={patient_local}越界。"
            )
        ...
        if int(row.development_global_index) != expected_global:
            raise ValueError(
                f"{row.case_label}: development_global_index不一致。"
            )
        if int(row.fold_id) != expected_fold:
            raise ValueError(
                f"{row.case_label}: fold_id不一致。"
            )
        ...
        if not np.all(
            ...
            raise ValueError(
                f"{row.case_label}: 并非全部0/1/2/3/4/5年Landmark均有效。"
            )

    # -------------------------------------------------------------------------
    # Exact Step11 calibrated risk reconstruction.
    # -------------------------------------------------------------------------
    exact_risk_rows = []
    maximum_selection_file_risk_difference = 0.0
    ...
    for case_row in selected_cases.itertuples(index=False):
        patient_local = int(case_row.patient_local)
        ...
        for landmark_index in PRIMARY_LANDMARK_INDICES:
            ...
            if long_row < 0:
                raise ValueError(
                    f"{case_row.case_label}: Landmark {landmark_month}月long row<0。"
                )
            ...
            if not np.isfinite(exact_risk):
                raise ValueError(
                    f"{case_row.case_label}: Landmark {landmark_month}月风险非有限。"
                )

            source_column = risk_column_map[landmark_month]
            if source_column in selected_cases.columns:
                source_value = float(
                    selected_cases.loc[
                        selected_cases["case_label"].eq(case_row.case_label),
                        source_column,
                    ].iloc[0]
                )
                maximum_selection_file_risk_difference = max(
                    maximum_selection_file_risk_difference,
                    abs(source_value - exact_risk),
                )

            exact_risk_rows.append(
                {
                    "case_label": str(case_row.case_label),
                    "trajectory_label": str(case_row.trajectory_label),
                    "patient_local": patient_local,
                    "development_global_index": int(case_row.development_global_index),
                    "fold_id": int(case_row.fold_id),
                    "landmark_index": landmark_index,
                    "landmark_month": landmark_month,
                    "landmark_year": float(landmark_month / 12.0),
                    "predicted_5y_risk": exact_risk,
                }
            )

    if maximum_selection_file_risk_difference > 5e-6:
        raise ValueError(
            "病例筛选文件中的风险与Step11精确重建不一致："
            f"max diff={maximum_selection_file_risk_difference:.3e}。"
        )

    exact_risk_df = pd.DataFrame(exact_risk_rows)

    # -------------------------------------------------------------------------
    # Standardize output columns expected by the original Step12I downstream
    # functions.  No patient selection happens here.
    # -------------------------------------------------------------------------
    selected_cases["risk_delta_first_to_last"] = (
        selected_cases["risk_last"].astype(float)
        - selected_cases["risk_first"].astype(float)
    )
    selected_cases["risk_abs_delta_first_to_last"] = np.abs(
        selected_cases["risk_delta_first_to_last"].to_numpy(dtype=float)
    )

    # Recompute exact six-landmark risk summaries so all downstream metadata are
    # tied to Step11 rather than copied from a plotting file.
    for case_label in ["Case A", "Case B", "Case C"]:
        sub = (
            exact_risk_df.loc[
                exact_risk_df["case_label"].eq(case_label)
            ]
            .sort_values("landmark_month")
        )
        risks = sub["predicted_5y_risk"].to_numpy(dtype=float)
        idx = selected_cases["case_label"].eq(case_label)
        selected_cases.loc[idx, "risk_first"] = risks[0]
        selected_cases.loc[idx, "risk_last"] = risks[-1]
        selected_cases.loc[idx, "risk_mean"] = float(np.mean(risks))
        selected_cases.loc[idx, "risk_sd"] = float(np.std(risks, ddof=0))
        selected_cases.loc[idx, "risk_min"] = float(np.min(risks))
        selected_cases.loc[idx, "risk_max"] = float(np.max(risks))
        selected_cases.loc[idx, "risk_range"] = float(np.ptp(risks))
        selected_cases.loc[idx, "risk_delta_first_to_last"] = float(risks[-1] - risks[0])
        selected_cases.loc[idx, "risk_abs_delta_first_to_last"] = float(abs(risks[-1] - risks[0]))

    selected_cases["selection_uses_outcome"] = False
    selected_cases["selection_uses_attribution"] = False
    selected_cases["selection_uses_clinical_values"] = True
    selected_cases["selection_source"] = str(CLINICAL_CASE_SELECTION_FILE)
    selected_cases["selection_strategy"] = (
        "prespecified clinically coherent phenotype subgroup; representative "
        "patient closest to subgroup multivariable median profile"
    )

    # Outcome is appended only after the already-frozen selection.
    last_landmark_index = int(PRIMARY_LANDMARK_INDICES[-1])
    selected_cases[
        ...
        for patient_local in selected_cases["patient_local"]
    ]

    # Preserve the full clinical candidate audit for transparency when present.
    if CLINICAL_FULL_CANDIDATE_AUDIT_FILE.exists():
        clinical_candidate_audit = pd.read_csv(
            CLINICAL_FULL_CANDIDATE_AUDIT_FILE,
            encoding="utf-8-sig",
        )
        clinical_candidate_audit.to_csv(
    ...
    # -------------------------------------------------------------------------
    # Expand 3 selected patients to 18 OOF local-IG origins.
    # -------------------------------------------------------------------------
    rows = []
    case_lookup = selected_cases.set_index("patient_local")[[
        "case_label",
        "trajectory_label",
    ]].to_dict(orient="index")

    for patient_local in selected_cases["patient_local"].astype(int):
        for landmark_index in PRIMARY_LANDMARK_INDICES:
            landmark_index = int(landmark_index)
            landmark_month = int(LANDMARK_MONTHS[landmark_index])
            ...
            rows.append(
                {
                    "case_label": case_lookup[patient_local]["case_label"],
                    "trajectory_label": case_lookup[patient_local]["trajectory_label"],
                    "patient_local": patient_local,
                    "development_global_index": int(common.development_idx[patient_local]),
                    "fold_id": int(common.development_fold_id[patient_local]),
                    "landmark_index": landmark_index,
                    "landmark_month": landmark_month,
                    "landmark_year": float(landmark_month / 12.0),
                    "event_within_5y": int(
    ...
    if len(table) != 18:
        raise ValueError(
            f"local IG origin应为18，实际={len(table)}。"
        )
    ...
    print(
        "\n临床一致代表病例已冻结并通过Step11风险重建审计 | "
        f"selection risk max diff={maximum_selection_file_risk_difference:.3e}"
    )
    print(
        selected_cases[[
            "case_label",
...
# =============================================================================
# 19A. Final Figure 3 — three clinically coherent patient trajectories
# =============================================================================

def _figure3_display_name(variable: str) -> str:
    mapping = {
        ...
        "WHOstage": "WHO stage",
        "TDF_NNRTI_3TC_FTC_cum_month": "TDF NNRTI cumulative exposure",
        "TDF_PI_3TC_FTC_cum_month": "TDF PI cumulative exposure",
        "nonTDF_PI_cum_month": "Non-TDF PI cumulative exposure",
        "BIC_FTC_TAF_cum_month": "BIC/FTC/TAF cumulative exposure",
        "EVGc_FTC_TAF_cum_month": "EVG/c/FTC/TAF cumulative exposure",
        "TDF_INSTI_3TC_FTC_cum_month": "TDF + INSTI cumulative exposure",
        "nonTDF_DTG_cum_month": "Non-TDF DTG cumulative exposure",
        "nonTDF_traditional_NNRTI_cum_month": "Traditional NNRTI cumulative exposure",
        "current_TDF_NNRTI_3TC_FTC": "Current TDF NNRTI",
        "current_TDF_PI_3TC_FTC": "Current TDF PI",
        "current_nonTDF_PI": "Current non-TDF PI",
        "current_BIC_FTC_TAF": "Current BIC/FTC/TAF",
        "current_EVGc_FTC_TAF": "Current EVG/c/FTC/TAF",
        "current_TDF_INSTI_3TC_FTC": "Current TDF + INSTI",
        "current_nonTDF_DTG": "Current non-TDF DTG",
        "current_nonTDF_traditional_NNRTI": "Current traditional NNRTI",
    }
    return mapping.get(str(variable), str(variable).replace("_", " "))


def plot_final_three_patient_clinical_ig_figure(
    risk_trajectory: pd.DataFrame,
    sample_clinical_df: pd.DataFrame,
) -> None:
    """
    Main patient-specific explanation figure.

    Per case:
      left   = exact cross-fit calibrated 5-year risk trajectory;
      middle = five raw longitudinal clinical trajectories used for transparent
               clinical reading (not selected by IG);
      right  = objectively selected Top-6 patient-specific local-IG variables,
               ranked by mean absolute local IG across all six landmarks.

    The heatmap shows SIGNED local IG in percentage points and therefore is a
    model contribution relative to the fold/landmark reference, not a causal
    effect estimate.
    """

    require_files([CLINICAL_TRAJECTORY_FILE])
    ...
    missing = sorted(required_clinical - set(clinical.columns))
    if missing:
        raise ValueError(
            "临床轨迹文件缺少字段："
            f"{missing}"
    ...
    case_titles = {
        "Case A": "Stable low risk",
        "Case B": "Increasing risk with renal deterioration",
        "Case C": "Persistently higher-risk, clinically stable profile",
    }
    case_colors = {
        "Case A": "#1B9E77",
    ...
    clinical_specs = [
        ("eGFR", "eGFR", "mL/min/1.73 m²", 1),
        ("Urea", "Urea", "", 1),
        ("HB", "Hemoglobin", "g/L", 0),
        ("CD4", "CD4", "cells/µL", 0),
        ("HIVRNA_log10", "HIV RNA", "log10", 2),
    ]

    # -------------------------------------------------------------------------
    # Audit clinical file against exact Step11 risk trajectory.
    # -------------------------------------------------------------------------
    clinical_risk_check = clinical[[
        "case_label",
        "patient_local",
        "landmark_month",
        "predicted_5y_risk",
        ...
        how="inner",
        suffixes=("_selection", "_step11"),
        validate="one_to_one",
    )

    if len(clinical_risk_check) != 18:
        raise ValueError(
            "临床轨迹与Step12I精确风险不能一一匹配18个origin。"
        )

    clinical_risk_diff = np.abs(
        clinical_risk_check["predicted_5y_risk_selection"].to_numpy(dtype=float)
        - clinical_risk_check["predicted_5y_risk_step11"].to_numpy(dtype=float)
    )
    if float(np.max(clinical_risk_diff)) > 5e-6:
        raise ValueError(
            "临床轨迹文件风险与Step11精确风险不一致："
            f"max diff={float(np.max(clinical_risk_diff)):.3e}。"
        )

    # -------------------------------------------------------------------------
    # Objectively select six local-IG variables per patient using all six
    # landmarks.  No sign-based or clinical-direction-based filtering.
    # -------------------------------------------------------------------------
    top_ig_rows = []
    heatmap_payload = {}

    for case_label in case_order:
        case_info = selected_case.loc[
            selected_case["case_label"].eq(case_label)
        ]
        if len(case_info) != 1:
            raise ValueError(f"{case_label}在case selection中不是1行。")
        patient_local = int(case_info.iloc[0]["patient_local"])

        ig_case = sample_clinical_df.loc[
            sample_clinical_df["patient_local"].eq(patient_local)
        ].copy()

        if ig_case["landmark_month"].nunique() != 6:
            raise ValueError(
                f"{case_label} local IG不是完整6个Landmark。"
            )

        ranking = (
            ig_case.groupby("clinical_variable", as_index=False)
            .agg(
                mean_abs_local_ig=("absolute_ig", "mean"),
                mean_signed_local_ig=("signed_ig", "mean"),
            )
            .sort_values(
                ["mean_abs_local_ig", "clinical_variable"],
                ascending=[False, True],
            )
            .head(TOP_LOCAL_HEATMAP_N)
            .reset_index(drop=True)
        )

        variables = ranking["clinical_variable"].astype(str).tolist()

        pivot = (
            ig_case.loc[
                ig_case["clinical_variable"].isin(variables)
            ]
            .pivot(
                index="clinical_variable",
                columns="landmark_month",
                values="signed_ig",
            )
            .reindex(index=variables, columns=LANDMARK_MONTHS)
        )

        if pivot.isna().any().any():
            raise ValueError(
                f"{case_label} Top local-IG heatmap存在缺失格。"
            )

        matrix_pp = 100.0 * pivot.to_numpy(dtype=float)
        heatmap_payload[case_label] = {
            "patient_local": patient_local,
            "variables": variables,
            "matrix_pp": matrix_pp,
        }

        for rank_index, row in ranking.iterrows():
            top_ig_rows.append(
                {
                    "case_label": case_label,
                    "patient_local": patient_local,
                    "rank": int(rank_index + 1),
                    "clinical_variable": str(row["clinical_variable"]),
                    "mean_abs_local_ig": float(row["mean_abs_local_ig"]),
                    "mean_signed_local_ig": float(row["mean_signed_local_ig"]),
                }
            )

    top_ig_source = pd.DataFrame(top_ig_rows)
    top_ig_source.to_csv(
        FINAL_PATIENT_TOP_IG_SOURCE,
        index=False,
        encoding="utf-8-sig",
    )
    ...
    all_heat_values = np.concatenate([
        payload["matrix_pp"].ravel()
        for payload in heatmap_payload.values()
    ])
    ig_limit = float(np.max(np.abs(all_heat_values)))
    if not np.isfinite(ig_limit) or ig_limit <= 0:
        ig_limit = 1.0

    # Slight headroom avoids the most extreme cell sitting exactly at cmap edge.
    ig_limit *= 1.03
    ig_norm = TwoSlopeNorm(
        vmin=-ig_limit,
        vcenter=0.0,
        vmax=ig_limit,
    )

    risk_pct_all = 100.0 * risk_trajectory["predicted_5y_risk"].to_numpy(dtype=float)
    risk_upper = max(5.0, float(np.nanmax(risk_pct_all)) * 1.16)
    risk_upper = math.ceil(risk_upper / 5.0) * 5.0

    # -------------------------------------------------------------------------
    # Figure layout: 3 cases × [risk | five raw clinical mini-trajectories | IG]
    # -------------------------------------------------------------------------
    fig = plt.figure(
        figsize=(23.0, 13.4),
        constrained_layout=True,
    ...
    for row_index, case_label in enumerate(case_order):
        patient_local = int(
            heatmap_payload[case_label]["patient_local"]
        )
        color = case_colors[case_label]

        risk_case = (
            risk_trajectory.loc[
                risk_trajectory["case_label"].eq(case_label)
            ]
            .sort_values("landmark_year")
        )
        clinical_case = (
            clinical.loc[
                clinical["case_label"].eq(case_label)
            ]
            .sort_values("landmark_year")
        )

        if len(risk_case) != 6 or len(clinical_case) != 6:
            raise ValueError(f"{case_label}不是6个完整Landmark。")

        # ---------------------------------------------------------------------
        # Left: exact calibrated 5-year risk trajectory.
        # ---------------------------------------------------------------------
        ax_risk = fig.add_subplot(outer[row_index, 0])
        x = risk_case["landmark_year"].to_numpy(dtype=float)
        y = 100.0 * risk_case["predicted_5y_risk"].to_numpy(dtype=float)
        ...
        for xv, yv in zip(x, y):
            ax_risk.annotate(
                f"{yv:.1f}%" if yv >= 1.0 else f"{yv:.2f}%",
                (xv, yv),
                xytext=(0, 6),
        ...
        ax_risk.set_xlim(-0.15, 5.15)
        ax_risk.set_ylim(0.0, risk_upper)
        ax_risk.set_xticks(LANDMARK_MONTHS / 12.0)
        ax_risk.grid(axis="y", linestyle="--", alpha=0.25, linewidth=0.6)
        ax_risk.set_ylabel("Predicted subsequent\n5-year CKD risk (%)")
        if row_index == 2:
            ax_risk.set_xlabel("Prediction landmark after ART (years)")

        case_info = selected_case.loc[
            selected_case["case_label"].eq(case_label)
        ].iloc[0]
        age_at_art = float(case_info.get("Age_baseline", np.nan))
        title_suffix = (
            f" | age at ART {age_at_art:.0f} y"
            if np.isfinite(age_at_art)
            else ""
        )
        ax_risk.set_title(
            f"{case_label}. {case_titles[case_label]}{title_suffix}",
            loc="left",
            fontweight="bold",
            fontsize=11.0,
            pad=8,
        ...
        # ---------------------------------------------------------------------
        # Middle: actual raw clinical trajectories.  Five separate y-axes avoid
        # mixing incompatible laboratory units in one plot.
        # ---------------------------------------------------------------------
        clinical_grid = outer[row_index, 1].subgridspec(
            1,
            len(clinical_specs),
            wspace=0.30,
        )

        for clinical_index, (variable, short_label, unit, decimals) in enumerate(clinical_specs):
            ax = fig.add_subplot(clinical_grid[0, clinical_index])
            values = clinical_case[variable].to_numpy(dtype=float)
            years = clinical_case["landmark_year"].to_numpy(dtype=float)
            ...
            ax.grid(axis="y", linestyle=":", alpha=0.22, linewidth=0.5)

            if row_index == 0:
                label = short_label + (f"\n({unit})" if unit else "")
                ax.set_title(label, fontsize=9.0, fontweight="bold", pad=5)

            if row_index == 2:
                ax.set_xlabel("ART year", fontsize=8.0)

            # Start → end actual values are explicitly printed; the small panels
            # are intended for clinical direction/magnitude reading, not effect estimation.
            finite_idx = np.where(np.isfinite(values))[0]
            if finite_idx.size > 0:
                first_value = values[finite_idx[0]]
                last_value = values[finite_idx[-1]]
                fmt = f"{{:.{decimals}f}}"
                ax.text(
                    0.02,
                    0.98,
                    f"{fmt.format(first_value)} → {fmt.format(last_value)}",
                    transform=ax.transAxes,
                    ha="left",
                    va="top",
                    fontsize=7.2,
                ...
                source_rows.append(
                    {
                        "case_label": case_label,
                        "patient_local": patient_local,
                        "landmark_year": float(year),
                        "panel": "clinical_trajectory",
                        "variable": variable,
        ...
        # ---------------------------------------------------------------------
        # Right: patient-specific local IG evolution.
        # ---------------------------------------------------------------------
        ax_ig = fig.add_subplot(outer[row_index, 2])
        payload = heatmap_payload[case_label]
        matrix_pp = payload["matrix_pp"]
        variables = payload["variables"]

        heat_image = ax_ig.imshow(
            matrix_pp,
            aspect="auto",
        ...
        ax_ig.set_xticks(np.arange(6))
        ax_ig.set_xticklabels(["0", "1", "2", "3", "4", "5"])
        ax_ig.set_yticks(np.arange(len(variables)))
        ax_ig.set_yticklabels([
            _figure3_display_name(variable)
            for variable in variables
        ], fontsize=7.6)
        ax_ig.set_title(
            "Top patient-specific local IG contributions",
            fontweight="bold",
            fontsize=9.6,
            pad=5,
        )
        if row_index == 2:
            ax_ig.set_xlabel("Prediction landmark after ART (years)")

        for i in range(matrix_pp.shape[0]):
            for j in range(matrix_pp.shape[1]):
                value = float(matrix_pp[i, j])
                relative = abs(value) / ig_limit
                text_color = "white" if relative > 0.58 else "black"
                ax_ig.text(
                    j,
                    i,
                    f"{value:+.2f}",
        ...
        ax_ig.tick_params(axis="x", labelsize=8.0)
    ...
    cbar.set_label(
        "Signed local IG contribution to predicted 5-year CKD risk\n(percentage points)",
        fontsize=9.0,
    )

    fig.suptitle(
        "Patient-specific dynamic CKD risk trajectories, clinical evolution, and local model contributions",
        fontsize=15.0,
        fontweight="bold",
    )
    ...
    fig.text(
        ...
        0.006,
        (
            "Clinical panels show raw aligned longitudinal values. Local IG is a model contribution relative to the "
            "fold/landmark reference and should not be interpreted as a causal effect. Top IG variables are selected "
            "objectively within each patient by mean absolute local IG across all six landmarks."
        ),
        ha="center",
        va="bottom",
        fontsize=8.3,
    ...
    print("\n最终Figure 3已生成：")
    print(" -", FINAL_PATIENT_FIGURE_PNG)
    print(" -", FINAL_PATIENT_FIGURE_PDF)
    print(" -", FINAL_PATIENT_FIGURE_SOURCE)
    print(" -", FINAL_PATIENT_TOP_IG_SOURCE)
...
def run() -> None:
    ...
    required = [STEP10E_DIR / "lstm_v2_summary.json", CLINICAL_CASE_SELECTION_FILE, CLINICAL_TRAJECTORY_FILE]
    if EXPLAIN_CALIBRATED:
    ...
    print("病例数：3；病例来自临床一致性预筛选；每名患者使用全部6个正式Landmark（0/1/2/3/4/5年）")
    ...
    plot_final_three_patient_clinical_ig_figure(
        risk_trajectory=exact_risk_trajectory,
        sample_clinical_df=sample_clinical_df,
    )
    ...
    summary = {
        ...
        "patient_dynamic_explanation": {
            ...
            "selection_uses_clinical_values": True,
