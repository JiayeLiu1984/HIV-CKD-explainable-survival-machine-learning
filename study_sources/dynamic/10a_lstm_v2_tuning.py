...
"""
Step 10D：LSTM-v2强化模型可恢复五折调参（单文件独立版）
...
不依赖Step10D_LSTM_v2_common.py，可作为.py脚本直接运行。
锁定测试集不读取。
"""
...
class LandmarkBalancedSurvivalLoss(nn.Module):
    ...
    def forward(
        ...
        if self.auxiliary_5y_weight > 0 and known_5y.any():
            auxiliary_sample = torch.nn.functional.binary_cross_entropy(
                risk_5y[known_5y].clamp(EPS, 1.0 - EPS),
                target_5y[known_5y],
                reduction="none",
            )
...
TARGET_COMPLETE_TRIAL_N = int(
    os.getenv("CKD_LSTM_V2_TRIALS", "30")
)
MAX_TOTAL_ATTEMPT_N = int(
    os.getenv("CKD_LSTM_V2_MAX_ATTEMPTS", "70")
)
BASE_SEED = int(
    os.getenv("CKD_LSTM_V2_SEED", "20260803")
)
IBS_NEAR_BEST_TOLERANCE = float(
    os.getenv("CKD_LSTM_V2_IBS_TOLERANCE", "0.00010")
)
SEARCH_SPACE_VERSION = "lstm_v2_hybrid_attention_20260803_v1"
...
LOG_FILE = OUTPUT_DIR / "step10d_live_progress.log"
PROGRESS_FILE = OUTPUT_DIR / "step10d_progress.csv"
DB_FILE = TUNING_DIR / "lstm_v2_optuna.sqlite3"
STORAGE_URL = f"sqlite:///{DB_FILE}"
STUDY_NAME = "lstm_v2_hybrid_attention_equal_weight_ibs"
LOCK_FILE = OUTPUT_DIR / "step10d_single_process.lock"

REFERENCE_LSTM_V1_MEAN_IBS = 0.02557902661806264

if TARGET_COMPLETE_TRIAL_N < 1:
    raise ValueError("TARGET_COMPLETE_TRIAL_N必须为正整数。")
...
def append_progress(
    ...
    mean_iauc: float | None = None,
    mean_uno_c: float | None = None,
    ...
    return normalize_parameters(
        {
            ...
            "projection_size": trial.suggest_categorical(
                "projection_size",
                [64, 96, 128],
            ),
            ...
            "pooling_mode": trial.suggest_categorical(
                "pooling_mode",
                [
                    "last_attention",
                    "last_attention_mean",
            ...
            "static_hidden": trial.suggest_categorical(
                "static_hidden",
                [32, 64],
            ),
            "summary_hidden": trial.suggest_categorical(
                "summary_hidden",
                [64, 96, 128],
            ),
            "horizon_embed_dim": trial.suggest_categorical(
                "horizon_embed_dim",
                [8, 16, 24],
            ),
            ...
            "auxiliary_5y_weight": trial.suggest_categorical(
                "auxiliary_5y_weight",
                [0.0, 0.10, 0.25],
            ),
            "ranking_weight": trial.suggest_categorical(
                "ranking_weight",
                [0.0, 0.05, 0.10],
            ),
            "smoothness_weight": trial.suggest_categorical(
                "smoothness_weight",
                [0.0, 1e-4, 5e-4],
            ),
        }
...
def enqueue_prespecified_trials(study: optuna.Study) -> None:
    ...
    trial_list = [
        {
            ...
            "projection_size": 96,
            "bidirectional": False,
            "pooling_mode": "last_attention_mean",
            "static_hidden": 32,
            "summary_hidden": 96,
...
def select_final_trial(
    complete_trials: list[optuna.trial.FrozenTrial],
) -> tuple[optuna.trial.FrozenTrial, dict[str, Any]]:
    valid_trials = [
        trial
        for trial in complete_trials
        if (
            trial.user_attrs.get("search_space_version")
            == SEARCH_SPACE_VERSION
            and np.isfinite(float(trial.value))
            and np.isfinite(
                float(trial.user_attrs.get("mean_iauc", np.nan))
            )
            and np.isfinite(
                float(trial.user_attrs.get("mean_uno_c", np.nan))
            )
        )
    ]
    if not valid_trials:
        raise RuntimeError("没有可用于最终选择的完整Trial。")

    minimum_ibs = min(float(trial.value) for trial in valid_trials)
    eligible = [
        trial
        for trial in valid_trials
        if float(trial.value) <= (
            minimum_ibs + IBS_NEAR_BEST_TOLERANCE
        )
    ]
    eligible.sort(
        key=lambda trial: (
    ...
    rule = {
        ...
        "ibs_near_best_tolerance": IBS_NEAR_BEST_TOLERANCE,
        "eligible_trial_n": len(eligible),
        "selection_rule": (
            "Among trials with mean IBS no more than "
            "minimum IBS + tolerance, choose highest mean iAUC; "
            "then highest mean Uno C; then lowest IBS."
        ),
    }
...
def run() -> None:
    ...
    print("\n" + "=" * 96)
    print("Step 10D：LSTM-v2强化模型五折调参")
    print("=" * 96)
    ...
    print("AMP：", use_amp)
    print("目标完整Trial数：", TARGET_COMPLETE_TRIAL_N)
    print("每个Trial：固定完整五折")
    print("早停指标：六Landmark等权平均IBS")
    print("历史强化特征：40 + 15缺失 + 15时间间隔 + 15变化量")
    print("锁定测试集：未读取")
    ...
    try:
        ...
        raise RuntimeError(
            "检测到另一个Step 10D进程正在运行。"
            "请先停止旧进程，避免重复写入Optuna数据库。"
        ) from exc
    ...
    enqueue_prespecified_trials(study)
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        append_progress(
            ...
            message=json.dumps(parameters, ensure_ascii=False),
        )
        ...
        try:
            for fold_id in range(N_SPLITS):
                ...
                progress_print(
                    f"Trial {trial.number} | 第{fold_id + 1}/5折开始"
                )
                ...
                metrics = result["landmark_metrics"].copy()
                metrics.insert(0, "fold_id", fold_id)
                metrics.insert(0, "trial_number", trial.number)
                fold_metric_frames.append(metrics)

                fold_ibs.append(float(summary["mean_ibs"]))
                fold_iauc.append(float(summary["mean_iauc"]))
                fold_uno_c.append(float(summary["mean_uno_c"]))
                fold_best_snapshot_n.append(
                    int(result["selected_snapshot_n"])
                )

                current_mean_ibs = float(np.mean(fold_ibs))
                fold_elapsed = time.time() - fold_start
                progress_print(
                    f"Trial {trial.number} | 第{fold_id + 1}/5折完成 | "
                    f"IBS={summary['mean_ibs']:.6f} | "
                    f"iAUC={summary['mean_iauc']:.6f} | "
                    f"UnoC={summary['mean_uno_c']:.6f} | "
                    f"快照数={result['selected_snapshot_n']} | "
                ...
                append_progress(
                    ...
                    trial_number=trial.number,
                    fold_id=fold_id,
                    mean_ibs=float(summary["mean_ibs"]),
                    mean_iauc=float(summary["mean_iauc"]),
                    mean_uno_c=float(summary["mean_uno_c"]),
                    elapsed_seconds=fold_elapsed,
                )

                trial.report(current_mean_ibs, step=fold_id)
                if trial.should_prune():
                    raise optuna.TrialPruned(
                        f"完成{fold_id + 1}折后中位数剪枝。"
                    )
            ...
            mean_ibs = float(np.mean(fold_ibs))
            mean_iauc = float(np.mean(fold_iauc))
            mean_uno_c = float(np.mean(fold_uno_c))

            trial.set_user_attr("mean_iauc", mean_iauc)
            trial.set_user_attr("mean_uno_c", mean_uno_c)
            trial.set_user_attr("fold_ibs", fold_ibs)
            trial.set_user_attr("fold_iauc", fold_iauc)
            trial.set_user_attr("fold_uno_c", fold_uno_c)
            trial.set_user_attr(
            ...
            trial.set_user_attr("locked_test_read", False)
            ...
            pd.concat(
                ...
                TUNING_DIR
                / f"trial_{trial.number:03d}_landmark_metrics.csv",
                index=False,
                encoding="utf-8-sig",
            )
            ...
            progress_print(
                ...
                f"平均IBS={mean_ibs:.6f} | "
                f"平均iAUC={mean_iauc:.6f} | "
                f"平均UnoC={mean_uno_c:.6f} | "
            ...
            progress_print(f"Trial {trial.number}已剪枝。")
            raise
    ...
    while True:
        complete_trials = [
            ...
            if (
                ...
                and trial.user_attrs.get("search_space_version")
                == SEARCH_SPACE_VERSION
            )
        ]
        if len(complete_trials) >= TARGET_COMPLETE_TRIAL_N:
            break
        if attempt_n >= MAX_TOTAL_ATTEMPT_N:
            raise RuntimeError(
                "已达到本次最大Trial尝试数，"
                "但完整Trial仍不足目标。"
            )
        ...
        progress_print(
            f"当前完整Trial数={complete_n}/"
            f"{TARGET_COMPLETE_TRIAL_N}"
        )
    ...
    selected_trial, selection_rule = select_final_trial(complete_trials)
    selected_parameters = normalize_parameters(selected_trial.params)

    study.trials_dataframe().to_csv(
        TUNING_DIR / "lstm_v2_optuna_trials.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    summary = {
        ...
        "objective": (
            ...
            "each fold uses IBS-based early stopping and best snapshot ensemble."
        ),
        "target_complete_trial_n": TARGET_COMPLETE_TRIAL_N,
        "complete_trial_n": len(complete_trials),
        ...
        "fold_ibs": selected_trial.user_attrs["fold_ibs"],
        "fold_iauc": selected_trial.user_attrs["fold_iauc"],
        "fold_uno_c": selected_trial.user_attrs["fold_uno_c"],
        "fold_selected_snapshot_n": selected_trial.user_attrs[
            "fold_selected_snapshot_n"
        ],
        "selection_rule": selection_rule,
        "reference_lstm_v1_mean_ibs": REFERENCE_LSTM_V1_MEAN_IBS,
        "selected_minus_reference_ibs": float(
            selected_trial.value - REFERENCE_LSTM_V1_MEAN_IBS
        ),
    ...
    print("LSTM-v2调参完成")
    print("=" * 96)
    print("选中Trial：", selected_trial.number)
    print("五折平均IBS：", f"{selected_trial.value:.6f}")
    print(
        "五折平均iAUC：",
        f"{selected_trial.user_attrs['mean_iauc']:.6f}",
    )
    print(
        "五折平均Uno C：",
        f"{selected_trial.user_attrs['mean_uno_c']:.6f}",
    )
    print("选中参数：", selected_parameters)
    print(
        "相对当前LSTM-v1调参IBS差值：",
        f"{selected_trial.value - REFERENCE_LSTM_V1_MEAN_IBS:+.6f}",
    )
    print("锁定测试集：未读取")
