...
def main():
    ...
    internal=pd.read_csv(WORK/'internal_validation/internal_test_metadata.csv')
    internal['risk']=1-np.load(WORK/'internal_validation/lstm_frozen_survival.npy')[:,-1]
    ext=pd.read_csv(WORK/'external_validation/external_metadata.csv')
    dev['risk']=1-np.load(WORK/'development_cv/lstm_calibrated_survival.npy')[:,-1]
    ...
    for lm in range(6):
        ...
        for label,m in [('internal_test',internal[internal.landmark_index==lm].copy()),('external_transported',e)]:
            groups=np.searchsorted(cuts,m.risk,side='right')
            panel=pd.read_csv(DATA/f'synthetic_{"development" if label=="internal_test" else "external"}.csv')
            ...
            for group in range(3):
                ...
                if label=='internal_test' and len(part):
    ...
    fig.suptitle('SYNTHETIC INTERNAL TEST — frozen LSTM risk groups');fig.savefig(out/'synthetic_risk_groups.png',dpi=160);plt.close(fig)
    ...
    table=pd.read_csv(WORK/'development_cv/landmark_performance.csv');fig,axes=plt.subplots(1,3,figsize=(12,3.7),constrained_layout=True)
    ...
    fig.suptitle('DEVELOPMENT CV — model comparison, not final internal evaluation');fig.savefig(out/'development_cv_model_comparison.png',dpi=160);plt.close(fig)
    dump_json(out/'scope.json',{'synthetic_only':True,'thresholds':'Development tertiles transferred unchanged to internal test and external predictions; descriptive only.','clinical_benefit_demonstrated':False})
