...
# Prespecified evaluation horizons
...
# =========================
max_followup = min(
    float(y_train["interval"].max()),
    float(y_test["interval"].max())
)

eval_horizons = [t for t in REQUESTED_HORIZONS if t < max_followup]

if len(eval_horizons) < 2:
    eval_horizons = np.linspace(
        y_test["interval"].quantile(0.2),
        y_test["interval"].quantile(0.8),
        5
    ).tolist()
...
auc_df.to_csv(
    os.path.join(OUTPUT_DIR, "time_dependent_auc.csv"),
    index=False,
    encoding="utf-8-sig"
)

brier_df.to_csv(
    os.path.join(OUTPUT_DIR, "brier_score.csv"),
    index=False,
    encoding="utf-8-sig"
)

calibration_df.to_csv(
    os.path.join(OUTPUT_DIR, "calibration_data.csv"),
    index=False,
    encoding="utf-8-sig"
)

dca_df_all.to_csv(
    os.path.join(OUTPUT_DIR, "dca_data.csv"),
    index=False,
    encoding="utf-8-sig"
)
